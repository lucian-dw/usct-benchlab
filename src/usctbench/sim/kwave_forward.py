...
def _simulate_external_kwave_forward(
    ...
    *,
    ...
    c0 = float(simulation.get("reference_sound_speed_mps", property_case.metadata.get("reference_sound_speed_mps", 1500.0)))
    root = Path(_expand(str(outputs.get("external_root", outputs.get("root", "runs/usctbench_runs/kwave_real_forward")))))
    root.mkdir(parents=True, exist_ok=True)
    mat_key = str(simulation.get("mat_key", "usct_property"))
    property_mat = Path(_expand(str(outputs.get("property_mat", root / "property_case.mat"))))
    water_mat = Path(_expand(str(outputs.get("water_mat", root / "water_case.mat"))))
    object_dataset = Path(_expand(str(outputs.get("object_dataset", root / "object_dataset.mat"))))
    water_dataset = Path(_expand(str(outputs.get("water_dataset", root / "water_dataset.mat"))))
    object_siminfo = Path(_expand(str(outputs.get("object_siminfo", root / "object_siminfo.mat"))))
    water_siminfo = Path(_expand(str(outputs.get("water_siminfo", root / "water_siminfo.mat"))))
    object_summary = Path(_expand(str(outputs.get("object_summary", root / "object_forward_summary.json"))))
    water_summary = Path(_expand(str(outputs.get("water_summary", root / "water_forward_summary.json"))))
