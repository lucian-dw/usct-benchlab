...
from usctbench.algorithms.ray import StraightRayCGLSAlgorithm, StraightRayProjector, StraightRaySARTAlgorithm, StraightRaySIRTAlgorithm
...
def test_ray_sound_speed_algorithms_run(synthetic_case):
    config = AlgorithmConfig(parameters={"iterations": 3, "subsets": 4, "inner_iterations": 4, "outer_iterations": 1})

    for algorithm in (StraightRayCGLSAlgorithm(), StraightRaySIRTAlgorithm(), StraightRaySARTAlgorithm(), BentRayGNAdapter(), RWaveAdapter()):
...
def test_attenuation_algorithm_runs_on_log_amplitude_case():
    ...
    result = AttenuationSIRTAlgorithm().run(case, AlgorithmConfig(parameters={"iterations": 3}))
