...
from usctbench.algorithms.fwi.adapter import KWaveFWIAdapterAlgorithm, read_kwave_fwi_result
...
def test_kwave_adapter_ingests_result_file(synthetic_case, tmp_path):
    ...
    with h5py.File(result_path, "w") as handle:
        ...
        handle.create_dataset("C_INTERP", data=np.asarray(synthetic_case.ground_truth.sound_speed_mps))
        handle.create_dataset("LOSS_ITER", data=np.array([3.0, 2.0, 1.0]))

    external = read_kwave_fwi_result(result_path)
    result = KWaveFWIAdapterAlgorithm().run(synthetic_case, AlgorithmConfig(parameters={"result_path": str(result_path)}))
