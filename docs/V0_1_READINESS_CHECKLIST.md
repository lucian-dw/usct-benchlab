...
|---|---|---|
| Local `pytest -q` passes | 30 tests pass locally on `codex-agent0-skeleton` | Passing |
| A100 `pytest -q` passes | 30 tests pass with `/home/wudalong/miniconda3/bin/python` | Passing |
...
| MATLAB/r-Wave adapter callable or skipped clearly | adapter tests verify `skipped` and failure report | Passing skip path |
